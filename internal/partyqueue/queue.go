...
// Queue batches party IDs and, on each tick, asks the database which of them
...
// Enqueue submits a party ID to the queue. If the buffer is full the ID is
...
func (q *Queue) flush(ctx context.Context) {
	if dropped := q.dropped.Swap(0); dropped > 0 {
		slog.Warn("party-exists queue saturated, dropped ids", "count", dropped)
	}
...
drain:
	...
	nonExisting, err := q.colls.Trackers.Party.Exists(ctx, ids)
	if err != nil {
		slog.Error("Failed checking if party IDs exist", "error", err)
		return
	}

	for _, id := range nonExisting {
		err = q.colls.Trackers.Party.CreateEmpty(ctx, id)
		if err != nil {
			slog.Error("Failed creating empty party tracker", "id", id.Hex(), "error", err)
		}
	}
}
